...
1. Run `adkit verify` and confirm the account is healthy. If not, stop and
   explain what to fix.
2. If the brief implies specific interests, run `adkit targeting search` to find
   the interest IDs.
3. Write a brief file at `briefs/<slug>.yaml` following
   `examples/briefs/example.yaml`: campaign objective and budget, one ad set per
   audience with targeting, and one or more ads with real copy and a `link` (or a
   `generate` block if they want AI creative).
